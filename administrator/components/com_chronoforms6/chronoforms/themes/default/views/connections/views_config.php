...
<div class="ui segment blue dragged_item">
	...
	<i class="icon copy green link copy_link" data-hint="<?php el('Copy'); ?>" data-block="view"></i>
	...
	<div class="ui popup top left transition hidden G2-static-popup" id="save-view-<?php echo $count; ?>" style="min-width:300px;">
		<div class="ui form">
			<div class="field">
				...
				<input type="text" name="title[]" value="<?php echo !empty($view) ? $name : $name.$count; ?>">
			</div>
			<div class="field">
				<div class="ui button black compact icon fluid G2-dynamic save_block" data-dtask="send/closest:.dragged_item" data-result="after/closest:.dragged_item" data-complete-message="<?php el("Block saved successfully."); ?>" data-url="<?php echo r2('index.php?ext=chronoforms&cont=connections&act=save_block&tvout=view&type=views'); ?>"><?php el('Save block'); ?></div>
			</div>
		</div>
	</div>
	...
	<?php if(!empty($vwsections)): ?>
		<?php foreach($vwsections as $ename => $ecolor): ?>
			...
			<div class="ui segment top attached secondary inverted <?php echo $ecolor; ?>" style="padding:0.2em 1em;">
				<?php echo $ename; ?>
				<i class="icon <?php if(!empty($this->data('Connection.views.'.$count.'.'.$ename.'.minimized'))):?>maximize<?php else: ?>minimize<?php endif; ?> white link minimize_link" data-hint="<?php el('Minimize/Maximize'); ?>" data-named="<?php echo $type.$count; ?>/<?php echo $ename; ?>"></i>
			</div>
