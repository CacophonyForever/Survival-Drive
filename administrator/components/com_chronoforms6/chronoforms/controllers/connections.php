...
class Connections extends \G2\L\Controller {
	...
	function index(){
		...
		if(!empty($this->data('search'))){
			$this->Connection->where('title', '%'.$this->data('search').'%', 'LIKE');
			$this->Connection->where('OR');
			$this->Connection->where('alias', '%'.$this->data('search').'%', 'LIKE');
			$this->Connection->where('OR');
			$this->Connection->where('description', '%'.$this->data('search').'%', 'LIKE');
		}
		...
		if(!empty($db_tables) AND !in_array($Block->tablename, $db_tables)){
			\G2\L\Env::redirect(r2('index.php?ext=chronoforms&cont=installer'));
		}
	}
	...
	function edit(){
		...
		//load saved blocks
		$Block = new \G2\A\E\Chronoforms\M\Block();
		$blocks = $Block->fields(['id', 'title', 'type', 'group', 'desc'])->where('published', 1)->select('all');
		$blocks_views = [];
		$blocks_functions = [];
